...
const navigationMenuTriggerStyle = cva(
  "group inline-flex h-9 w-max items-center justify-center rounded-full px-4 py-2 text-sm font-medium transition-colors bg-transparent text-foreground/70 hover:bg-accent hover:text-accent-foreground focus:bg-accent focus:text-accent-foreground focus:outline-none disabled:pointer-events-none disabled:opacity-50 data-[active]:bg-accent/50 data-[state=open]:bg-accent/50"
)
...
const NavigationCard = React.forwardRef(
  ({ className, title, children, icon: Icon, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'group relative flex h-full flex-col justify-between rounded-lg bg-background p-6 transition-all duration-300 hover:bg-muted/50',
        className,
      )}
      {...props}
    >
      <div>
        <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-lg border-2 border-border bg-muted text-foreground">
