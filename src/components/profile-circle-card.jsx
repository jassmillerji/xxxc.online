...
const ProfileCircleCard = ({ profile, href = "#" }) => {
  const profileLink = href === '#' ? `/pornstars/${profile.name.toLowerCase().replace(/ /g, '-')}`: href;
  return (
    <div className="group flex-shrink-0 text-center w-28 md:w-32">
      ...
      <div className="mt-3">
        <Link href={profileLink} className="text-sm font-bold text-white hover:text-accent truncate block">
          {profile.name}
        </Link>
      </div>
    </div>
